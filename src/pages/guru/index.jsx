...
export default function GuruManajemenKelasPage() {
  ...
  // -- DIUBAH: State dan Ref untuk menu profil dipindahkan ke sini --
  ...
  const handleKelasSubmit = async (formData) => {
    ...
    try {
      if (isEditing) {
        await updateDoc(doc(db, 'kelas', formData.id), { namaKelas: formData.namaKelas, keterangan: formData.keterangan });
      } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-400 to-orange-600 p-4 sm:p-6 relative">
        {/* -- DIUBAH: Header diganti menjadi menu profil mengambang di pojok kanan atas -- */}
...
// (Kode untuk modal tidak berubah dan tetap dipertahankan)
function KelasModal({ show, onClose, onSubmit, kelas }) {
    const [namaKelas, setNamaKelas] = useState('');
    const [keterangan, setKeterangan] = useState('');
    const isEditing = !!kelas;
    ...
    const handleSubmit = (e) => {
        ...
        onSubmit({ id: kelas?.id, namaKelas, keterangan });
    };
            ...
            <div className="bg-white rounded-lg p-7 w-full max-w-lg relative animate-fade-in-up">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <input type="text" value={namaKelas} onChange={(e) => setNamaKelas(e.target.value)} required className="w-full border px-4 py-2 rounded-md focus:ring-orange-500 focus:border-orange-500" placeholder="Contoh: Matematika Kelas 9" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium mb-1">Keterangan</label>
                        <textarea value={keterangan} onChange={(e) => setKeterangan(e.target.value)} rows={3} required className="w-full border px-4 py-2 rounded-md focus:ring-orange-500 focus:border-orange-500" placeholder="Deskripsi singkat tentang kelas ini" />
                    </div>
                    <div className="flex justify-end space-x-3 pt-3">
                        <button type="button" onClick={onClose} className="px-5 py-2 bg-gray-200 rounded-lg hover:bg-gray-300">Batal</button>
