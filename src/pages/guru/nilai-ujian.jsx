...
export default function BeriNilaiUjianPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(submissionsQuery, (snapshot) => {
        ...
        fetchedSubmissions.forEach(sub => {
            initialSkor[sub.id] = sub.skorTambahan || 0;
        });
  ...
  const handleSkorTambahanChange = (submissionId, value) => {
    const newSkor = value === '' ? 0 : Math.max(0, Number(value));
    setSkorTambahan(prev => ({ ...prev, [submissionId]: newSkor }));
  };
  ...
  const handleSimpanSemuaNilai = async () => {
    ...
    submissions.forEach(sub => {
      const skorOtomatis = sub.score || 0;
      const skorTambahanInput = skorTambahan[sub.id] || 0;
      const nilaiAkhir = Math.min(100, skorOtomatis + skorTambahanInput);

      const resultDocRef = doc(db, "examSubmissions", sub.id);
      
      batch.update(resultDocRef, {
        skorTambahan: skorTambahanInput,
        nilai: nilaiAkhir,
    ...
    try {
      await batch.commit();
      showCustomAlert('Semua nilai berhasil disimpan!', 'success');
    } catch (error) {
  ...
  return (
    <MainLayout>
      <main className="flex-1 md:ml-64 md:pt-16 p-4 md:p-6 lg:p-8 bg-gradient-to-br from-slate-50 to-gray-100 min-h-screen mt-15">
            ...
            <div className={`mt-8 overflow-x-auto rounded-lg shadow-md border border-gray-200 ${hasMounted ? 'animate-fade-in-up' : 'opacity-0'}`} style={{ animationDelay: '0.3s' }}>
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {loadingSubmissions ? (
                    <tr><td colSpan="5" className="text-center p-10 text-gray-500"><Loader className="animate-spin inline-block mr-2" /> Memuat data pengumpulan...</td></tr>
                  ) : submissions.length === 0 ? (
                    <tr><td colSpan="5" className="text-center p-10 text-gray-500">Belum ada siswa yang mengerjakan ujian ini.</td></tr>
                  ) : (
                    submissions.map((sub) => (
                      <tr key={sub.id} className="hover:bg-gray-50/50 transition-colors duration-150">
                        ...
                        <td className="px-5 py-4 whitespace-nowrap text-sm text-center font-bold text-orange-600">
                          {Math.min(100, (sub.score || 0) + (skorTambahan[sub.id] || 0))}
                        </td>
                        <td className="px-5 py-4 whitespace-nowrap text-center text-sm font-medium">
                          <button onClick={() => handleOpenJawabanModal(sub)} className="text-blue-600 hover:text-blue-700 flex items-center justify-center gap-2 w-full">
              ...
              <div className="bg-gray-50 px-6 py-4 flex justify-end">
                <button 
                    onClick={handleSimpanSemuaNilai}
                    disabled={isSubmitting || loadingSubmissions || submissions.length === 0}
                    className="flex items-center space-x-2 px-5 py-2.5 rounded-lg shadow-md text-sm font-medium transition-colors bg-orange-500 text-white hover:bg-orange-600 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                    <Save size={16} />
                    <span>{isSubmitting ? 'Menyimpan...' : 'Simpan Semua Nilai'}</span>
                </button>
              </div>
            </div>
          )}
        ...
        {/* --- DIUBAH: Modal Lihat Jawaban sekarang menampilkan semua tipe soal --- */}
