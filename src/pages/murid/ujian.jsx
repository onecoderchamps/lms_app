...
export default function UjianMuridPage() {
  ...
  const fetchUserClasses = async (uid) => {
    try {
      ...
      const classIds = enrollmentsSnapshot.docs.map(
        (doc) => doc.data().kelasId
      );
  ...
  useEffect(() => {
    ...
    setLoading(true);
    // --- DIUBAH: Mengurutkan berdasarkan 'date' bukan 'deadline' ---
    const q = query(
      collection(db, "ujian"),
      where("kelas", "in", userClasses),
      orderBy("date", "desc") // FIX: Menggunakan field 'date' yang benar
    );
  ...
  const getUjianStatus = (ujian, resultData) => {
    ...
    if (now < ujianStartDateTime) {
      ...
      if (diffMinutes <= 60 && diffMinutes > 0) {
        return {
          text: `Mulai dalam ${diffMinutes} mnt`,
      ...
      return {
        ...
        action: "tunggu",
        icon: <BookText size={36} strokeWidth={1.5} />,
      };
      ...
      if (
        resultData &&
        resultData.nilai !== null &&
        resultData.nilai !== undefined
      ) {
        return {
          text: "Sudah Dinilai",
      ...
      return {
        ...
        action: "selesai",
        icon: <CheckSquare size={36} strokeWidth={1.5} />,
      };
    }
  };
    ...
    <MainLayout>
      <main className="flex-1 md:ml-64 md:pt-16 p-4 md:p-6 lg:p-8 bg-gradient-to-br from-slate-50 to-gray-100 min-h-screen mt-15">
        <div className="max-w-full mx-auto bg-white rounded-xl shadow-xl p-6 md:p-8">
          <div
            className={`mb-8 ${hasMounted ? "animate-fade-in-up" : ""}`}
          ...
          {loading ? (
            ...
            >
              ...
              <p className="text-sm text-gray-400 mt-2">
                Hubungi guru Anda jika ada pertanyaan.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-6">
              {ujians.map((ujian, index) => {
                ...
                return (
                  <div
                    key={ujian.id}
                    className={`bg-white rounded-xl shadow-lg overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1.5 flex flex-col ${
                      hasMounted ? "animate-fade-in-up" : ""
                    }`}
                    style={{ animationDelay: `${index * 0.05 + 0.2}s` }}
                  >
                    <div
                      className={`relative h-36 flex flex-col items-center justify-center p-4 text-white bg-gradient-to-br ${statusInfo.color}`}
                    >
                      {statusInfo.icon}
                    ...
                    <div className="p-5 flex flex-col flex-grow">
                      <p
                        className="font-semibold text-md text-gray-800 mb-1.5 leading-tight flex-grow min-h-[3.5rem] line-clamp-3"
                        title={ujian.name}
                      >
                        {ujian.name}
                      </p>
                      ...
                      <div className="flex items-center text-gray-600 text-xs mb-1">
                        ...
                        />
                        <span>
                          {new Date(ujian.date).toLocaleDateString("id-ID", {
                            weekday: "long",
                            year: "numeric",
                            month: "long",
                            day: "numeric",
                      ...
                      <div className="flex items-center text-gray-600 text-xs mb-4">
                        ...
                        <span>
                          Pukul: {ujian.time} WIB (Durasi:{" "}
                          {ujian.durationMinutes} mnt)
                        </span>
                      </div>
                      <div className="mt-auto">
                        {statusInfo.action === "mulai" ? (
                          <a
                            onClick={() => {
                              router.push(`/murid/soal?ujianId=` + ujian.id);
                            }}
                          ...
                          >
                            {" "}
                            <PlayCircle size={16} className="mr-2" /> Mulai
                            Ujian Sekarang{" "}
                          </a>
                        ) : statusInfo.action === "lihat_nilai" ? (
                          <button
                            onClick={() =>
                              handleOpenNilaiModal(ujian, resultData)
                            }
                          ...
                          <button
                            className="flex items-center justify-center bg-gray-200 text-gray-500 px-4 py-2.5 rounded-lg shadow-sm cursor-not-allowed text-sm font-medium w-full"
                            disabled
                          >
                            {" "}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
            <div className="bg-white rounded-xl shadow-xl p-7 w-full max-w-lg relative animate-fade-in-up">
              ...
              <div className="text-center">
                <span className="inline-block p-3 bg-green-100 text-green-600 rounded-full mb-3">
                  <Star size={32} />
                </span>
              ...
              <div className="flex justify-end space-x-3 pt-6">
                <button
                  type="button"
                  onClick={() => setShowNilaiModal(false)}
                  className={`px-5 py-2.5 ${primaryButtonColor} ${primaryButtonTextColor} rounded-lg shadow-md text-sm font-medium`}
                >
                  Tutup
                </button>
              </div>
