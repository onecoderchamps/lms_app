...
export default function TugasMuridPage() {
  ...
  const handleOpenSubmitModal = (tugasItem) => {
    ...
    setSubmissionLink(tugasItem.submissionData?.submissionUrl || '');
    setShowSubmitModal(true);
  };

  const handleActualSubmit = async () => {
    if (!submissionLink.trim()) {
      showCustomAlert('Silakan masukkan link tugas Anda.', 'error');
      return;
    }
    ...
    try {
      ...
      if (docSnap.exists() && docSnap.data().status === 'Sudah Dikumpulkan') {
          showCustomAlert('Anda sudah pernah mengumpulkan tugas ini!', 'error');
      ...
      await setDoc(submissionRef, {
        ...
        muridId: user.uid,
        namaSiswa: user.namaLengkap || 'Nama Tidak Ada',
        ...
        nilai: null,
        komentarGuru: ''
      }, { merge: true }); // Gunakan merge untuk update jika dokumen sudah ada (misal, submit ulang)
        ...
        <div className="max-w-full mx-auto bg-white rounded-xl shadow-xl p-6 md:p-8">
          <div className={`mb-8 ${hasMounted ? 'animate-fade-in-up' : 'opacity-0'}`} style={{ animationDelay: '0.1s' }}>
            ...
            {Object.keys(classDetails).length > 0 && (
                <p className="mt-1 text-orange-600 font-semibold">
                    {Object.keys(classDetails).length === 1
                        ? `Kelas: ${Object.values(classDetails)[0]}`
          ...
          {loading ? (
            ...
            <div className={`text-center py-16 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300 ${hasMounted ? 'animate-fade-in-up' : 'opacity-0'}`} style={{ animationDelay: '0.2s' }}>
              <p className="text-xl font-medium text-gray-500">Hebat! Tidak ada tugas yang perlu dikerjakan.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {combinedTugas.map((item, index) => (
                ...
                >
                  <div className={`relative h-10 flex items-center justify-center p-4 text-white font-semibold text-sm
                    ${item.status === 'Belum Selesai' ? 'bg-red-500' :
                      item.status === 'Terlambat' ? 'bg-gray-500' :
                      item.status === 'Sudah Dikumpulkan' ? 'bg-yellow-500' :
                      'bg-green-500'
                    }`}
                  >
                    {item.status}
                  ...
                  <div className="p-5 flex flex-col flex-grow">
                    ...
                    {item.status === 'Sudah Dinilai' && (
                      <div className="flex items-center text-xs mb-3">
                        ...
                        <span className="font-semibold text-green-600">Nilai Anda: {item.submissionData?.nilai}</span>
                      </div>
                    )}
                    
                    <div className="mt-auto space-y-2.5 pt-2 border-t">
                      ...
                      {item.status === 'Belum Selesai' || item.status === 'Terlambat' ? (
                        ...
                        <button onClick={() => handleOpenSubmitModal(item)}
                          className="flex items-center justify-center px-4 py-2.5 rounded-lg shadow-md transition duration-300 text-sm font-medium w-full bg-yellow-500 text-white"
                        >
                          <Hourglass size={16} className="mr-2" /> Ganti Jawaban
                        </button>
                      ) : (
        ...
        {showSubmitModal && currentSubmittingTugas && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
            <div className="bg-white rounded-xl shadow-xl p-7 w-full max-w-lg relative animate-fade-in-up">
              ...
              <div>
                <label htmlFor="submissionLink" className="block text-sm font-medium text-gray-700 mb-2">Link Google Drive atau File Lainnya</label>
                <div className="relative">
                  <LinkIcon className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={20} />
                  <input type="url" id="submissionLink" placeholder="Salin dan tempel link tugas Anda di sini..."
                    value={submissionLink} onChange={(e) => setSubmissionLink(e.target.value)}
                    className={`w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg shadow-sm ${inputFocusColor} transition`}
                  />
                </div>
                <p className="text-xs text-gray-500 mt-2">Pastikan link yang Anda bagikan sudah diatur agar "Siapa saja yang memiliki link" bisa melihat.</p>
              </div>
              <div className="flex justify-end space-x-3 pt-6">
                ...
                <button type="button" onClick={handleActualSubmit} disabled={!submissionLink.trim() || isSubmitting}
                  className={`flex items-center space-x-2 px-5 py-2.5 rounded-lg shadow-md text-sm font-medium transition-colors disabled:opacity-50 ${!submissionLink.trim() ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : `${primaryButtonColor} ${primaryButtonTextColor}`}`}
                > 
