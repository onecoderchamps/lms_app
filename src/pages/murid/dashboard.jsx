...
// --- PERUBAHAN DI SINI: Import ikon yang relevan ---
import { BookText, ListChecks, Edit, Loader, AlertTriangle, Video, FileText, CalendarClock, BookOpenCheck } from 'lucide-react';
import DashboardCalendar from './component/calendar'; // Ini adalah komponen yang Anda berikan sebelumnya, tidak ada perubahan di sini
    ...
    useEffect(() => {
        ...
        const fetchDashboardData = async (muridId) => {
            try {
                ...
                const ids = enrollmentsSnapshot.docs.map(d => d.data().kelasId); // Mengambil kelasId yang diikuti murid
                ...
                const tugasQuery = query(collection(db, 'tugas'), where('kelasId', 'in', ids), where('deadline', '>=', Timestamp.fromDate(today))); // Asumsi 'kelasId' untuk tugas juga
                const ujianQuery = query(collection(db, 'ujian'), where('kelasId', 'in', ids), orderBy('date', 'asc')); // Asumsi 'kelasId' untuk ujian juga
                const materiQuery = query(collection(db, 'materi'), where('kelas', 'in', ids), orderBy('createdAt', 'desc'), limit(4)); // Tetap 'kelas' jika materi pakai 'kelas'

                const pengumumanQuery = query(collection(db, 'pengumuman'), where('kelasId', 'in', ids), orderBy('createdAt', 'desc'), limit(1));
                ...
                if (!ujianSnap.empty) {
                    ...
                    if (futureExams.length > 0) {
                        ...
                        const kelasUjianRef = doc(db, 'kelas', ujian.kelasId); // Asumsi 'kelasId' untuk referensi kelas ujian
                    ...
                    <div className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <DashboardCard
                                ...
                                icon={BookText} // Mengganti ikon ke BookText (lebih relevan untuk tugas)
                            ...
                            <DashboardCard
                                ...
                                icon={BookOpenCheck} // Mengganti ikon ke BookOpenCheck (lebih relevan untuk ujian)
